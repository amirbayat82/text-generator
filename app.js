...
let generate_btn = document.querySelector(".text-generator");
let addedtext = document.querySelector("#result");
...
generate_btn.addEventListener("click", (e) => {
  let amount = document.querySelector(".counter").value;
  e.preventDefault();
  addedtext.innerHTML ='';
  for(let i = 0; i < amount ; i++){
    let randomnumber = getRandomInt(amount);
    if(randomnumber < dummyText.length){
      addedtext.innerHTML +=dummyText[randomnumber] + '<br><br><br>';
    }
  }
});
